...
class StudentMicroservice {
    ...
    private lateinit var studentMicroservice: ServerSocket
    ...
    private fun subscribeToMessageManager() {
        try {
            ...
            println("M-am conectat la MessageManager ca ascultator!")
        } catch (e: Exception) {
    ...
    public fun run() = runBlocking {
        launch(Dispatchers.IO) {
            try {
                ...
                println("StudentMicroservice asculta pe portul: ${messageManagerSocket.localPort}")
        ...
        launch(Dispatchers.IO) {
            val studentServerSocket = ServerSocket(STUDENT_PORT)
            println("StudentMicroservice asteapta cereri pe portul: ${studentServerSocket.localPort}")
            ...
            while (isActive) {
                ...
                launch(Dispatchers.IO) {
                    ...
                    //in cazul in care nu se primeste nimic de la interfata grafica se opreste thread-ul
                    val receivedLine = clientBufferReader.readLine() ?: return@launch

                    val parts = receivedLine.split("|", limit = 2)
                    val destinatar = parts[0]
                    val receivedQuestion = parts[1]

                    val interograreSocket = Socket(MESSAGE_MANAGER_HOST, MESSAGE_MANAGER_PORT)
                    interograreSocket.soTimeout = 3000

                    // Construim comanda pentru MessageManager in functie de destinatar
                    val mesajDeTrimis = if (destinatar == "ALL") {
                        "intrebare ${interograreSocket.localPort} $receivedQuestion\n"
                    } else {
                        // Daca e un IP specific, folosim intrebare_privata!
                        "intrebare_privata $destinatar $receivedQuestion\n"
                    }

                    println("Trimit catre MessageManager: $mesajDeTrimis")
                    interograreSocket.getOutputStream().write(mesajDeTrimis.toByteArray())
