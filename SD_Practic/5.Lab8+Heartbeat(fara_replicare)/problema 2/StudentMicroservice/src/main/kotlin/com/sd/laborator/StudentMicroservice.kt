        ...
        while (true) {
            ...
            thread {
                ...
                when(messageType) {
                    ...
                    "intrebare" -> {
                        if(messageBody == "ping") {
                            val pongMessage = "raspuns $messageDestination pong ${messageManagerSocket.localPort}\n"
                            messageManagerSocket.getOutputStream().write(pongMessage.toByteArray())
                            println("Am trimis PONG înapoi către Heartbeat.")
                        }
                        else {
