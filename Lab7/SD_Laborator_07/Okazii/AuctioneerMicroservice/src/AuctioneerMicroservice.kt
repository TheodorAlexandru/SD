...
class AuctioneerMicroservice {
    ...
    companion object Constants {
        ...
        const val numeJurnal = "auctioneer_microservice"
    }
    ...
    private fun receiveBids() {
        ...
        val receiveBidsSubscription = receiveBidsObservable.subscribeBy(
            onNext = {
                ...
                println(message)
                Jurnal.scrieInFisier(numeJurnal, "PENDING", "$message")
            ...
            onComplete = {
                ...
                Jurnal.scrieInFisier(numeJurnal, "PENDING", "Licitatia s-a incheiat! Se trimit ofertele spre procesare...")

                forwardBids()
            },
            onError = { println("Eroare: $it")
                Jurnal.scrieInFisier(numeJurnal, "ERROR", "Eroare: $it")
            }
        )
        ...
        try {
            ...
            while (!conectat) {
                try {
                    ...
                    println("MessageProcessor este offline! Astept 2 secunde...")
                    Thread.sleep(2000)
                }
            }
            ...
            subscriptions.add(Observable.fromIterable(bidQueue).subscribeBy(
                onNext = {
                    ...
                    Jurnal.scrieInFisier(numeJurnal, "DONE", "Am trimis mesajul catre bidder: $it")
                },
                onComplete = {
                    println("Am trimis toate ofertele catre MessageProcessor.")
                    Jurnal.scrieInFisier(numeJurnal, "DONE", "Am trimis toate ofertele catre MessageProcessor.")
            ...
            Jurnal.scrieInFisier(numeJurnal, "ERROR", "Nu ma pot conecta la MessageProcessor!")
    ...
    private fun finishAuction() {
        ...
        try {
            ...
            println("Asteptam BiddingProcessor-ul, dar a aparut o intarziere...")
            Jurnal.scrieInFisier(numeJurnal, "ERROR", "Nu ma pot conecta la MessageProcessor!")
        }
    ...
    fun run() {
        ...
        if (dateSalvate != null) {
            println("[RECOVERY] Licitatia anterioara a fost intrerupta! Curat starea...")
            Jurnal.scrieInFisier(numeJurnal, "RECOVERY", "Oferte pierdute la stadiul: $dateSalvate")

            bidQueue.clear()
            bidderConnections.clear()

            Jurnal.scrieInFisier(numeJurnal, "DONE", "Sistem resetat. Se deschide o noua licitatie curata.")
        }

        Jurnal.stergeDinFisier(numeJurnal)

        Jurnal.scrieInFisier(numeJurnal, "START", "Microserviciul a pornit pe portul ${auctioneerSocket.localPort}")
        receiveBids()
        Jurnal.scrieInFisier(numeJurnal, "FINISH", "Am terminat de executat tot.")

        if (!auctioneerSocket.isClosed) {
            auctioneerSocket.close()
        }
