package com.sd.laborator.controllers

import com.sd.laborator.interfaces.LocationSearchInterface
import com.sd.laborator.interfaces.WeatherForecastInterface
import com.sd.laborator.pojo.WeatherForecastData
import com.sd.laborator.services.AlarmService
import com.sd.laborator.services.BlackzoneService
import com.sd.laborator.services.LocationSearchService
import com.sd.laborator.services.TimeService
import org.springframework.beans.factory.annotation.Autowired
import org.springframework.stereotype.Controller
import org.springframework.web.bind.annotation.PathVariable
import org.springframework.web.bind.annotation.RequestMapping
import org.springframework.web.bind.annotation.RequestMethod
import org.springframework.web.bind.annotation.ResponseBody

@Controller
class WeatherAppController {
    @Autowired
    private lateinit var locationSearchService: LocationSearchInterface

    @Autowired
    private lateinit var weatherForecastService: WeatherForecastInterface

    @Autowired
    private lateinit var alarmService : AlarmService

    @Autowired
    private lateinit var blackzoneService : BlackzoneService


    @RequestMapping("/getforecast/{location}", method = [RequestMethod.GET])
    @ResponseBody
    fun getForecast(@PathVariable location: String): String {

        if(blackzoneService.isServerLocationBlacklisted()) {
            val serverZone = blackzoneService.getServerLocation()
            return "Nu este permis accesul la astfel de informații pentru zona $serverZone."
        }

        val coordonate = locationSearchService.getLatAndLong(location)

        // dacă locaţia nu a fost găsită, răspunsul va fi corespunzător
        if (coordonate == null) {
            return "Nu s-au putut gasi date meteo pentru cuvintele cheie \"$location\"!"
        }

        // pe baza numelui de locaţie si a coordonatelor se interoghează al doilea serviciu care returnează datele meteo
        // încapsulate într-un obiect POJO
        val rawForecastData: WeatherForecastData = weatherForecastService.getForecastData(location, coordonate)

        val triggerAlarm = alarmService.checkTemperature(rawForecastData.currentTemp, 5, 30)

        if(triggerAlarm)
            return "temperatura este inafara limitelor acceptate"

        // fiind obiect POJO, funcţia toString() este suprascrisă pentru o afişare mai prietenoasă
        return rawForecastData.toString()
    }
}